...
import {
  BookOpen, Users, CalendarDays, Settings, Plus, Search,
  X, Edit2, Trash2, Download, Upload, Shield, Shuffle,
  Filter, Star, Clock, Package, Tag
} from "lucide-react";

// ─────────────────────────────────────────────────────────────────
// DEXIE (IndexedDB) – ersetzt window.storage, funktioniert offline
// ─────────────────────────────────────────────────────────────────
...
const readDataURL = (file) => new Promise((res,rej)=>{ const r=new FileReader(); r.onload=e=>res(e.target.result); r.onerror=rej; r.readAsDataURL(file); });
...
function buildTeams(players, perTeam, mode) {
  ...
  if (mode==="balanced") {
    ...
    [4,3,2,1].forEach(s=>{ players.filter(p=>p.strength===s).forEach((p,i)=>teams[i%n].players.push(p)); });
...
function useStorage(key, def) {
  ...
  const save = async (next) => {
    setData(next);
    try { await db.kv.put({ key, value: JSON.stringify(next) }); }
...
function useToast() {
  ...
  const Toasts=()=>(
    <div style={{position:"fixed",top:16,right:16,zIndex:9999,display:"flex",flexDirection:"column",gap:8,pointerEvents:"none"}}>
      {list.map(t=>(
        <div key={t.id} style={{padding:"10px 18px",borderRadius:10,fontSize:14,fontWeight:700,color:"white",
          background:t.type==="err"?"#ef4444":t.type==="warn"?"#f59e0b":"#16a34a",
  ...
  <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16}}>
    <div style={{background:C.card,borderRadius:18,width:"100%",maxWidth:wide?760:540,maxHeight:"92vh",overflow:"hidden",display:"flex",flexDirection:"column",boxShadow:"0 28px 80px rgba(0,0,0,.35)"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"20px 24px",borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
        ...
        <button onClick={onClose} style={{background:"none",border:"none",cursor:"pointer",color:C.muted,padding:6,borderRadius:8,fontSize:18,lineHeight:1}}>✕</button>
      </div>
...
function ExerciseForm({exercise, onSave, onClose}) {
  ...
  const [form,setForm]=useState({
    ...
    imageUrl:"",source:"",notes:"", ...exercise
  });
  const [matInput,setMatInput]=useState("");
  const imgRef=useRef();
  const set=(k,v)=>setForm(f=>({...f,[k]:v}));
  const addMat=(m)=>{ if(!m.trim()) return; if(!form.material.includes(m.trim())) set("material",[...form.material,m.trim()]); setMatInput(""); };
  const toggleTag=(t)=>set("tags",form.tags.includes(t)?form.tags.filter(x=>x!==t):[...form.tags,t]);
  const handleImage=async(e)=>{ const f=e.target.files?.[0]; if(!f) return; set("imageUrl",await readDataURL(f)); };
  const handleSave=()=>{ if(!form.title.trim()) return; onSave({...form,id:form.id||uid(),createdAt:form.createdAt||now(),updatedAt:now()}); };
  const chipStyle=(active)=>({padding:"4px 10px",borderRadius:20,border:`1.5px solid ${active?C.primary:C.border}`,background:active?C.accentL:"white",color:active?C.primary:C.muted,cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:"inherit"});
    ...
    <div>
      ...
      <div style={{marginBottom:14}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <input value={matInput} onChange={e=>setMatInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addMat(matInput)}
            placeholder="Eigenes Material + Menge..."
      ...
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:8,paddingTop:16,borderTop:`1px solid ${C.border}`}}>
        ...
        <Btn onClick={handleSave}>{isEdit?"Speichern":"Erstellen"}</Btn>
      </div>
    </div>
  );
}
...
// EXERCISE DETAIL
// ─────────────────────────────────────────────────────────────────
function ExerciseDetail({exercise:ex, onEdit, onDelete, onClose}) {
  return (
    <div>
      <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:16,flexWrap:"wrap"}}>
        ...
        <span style={{color:C.muted,fontSize:13,marginLeft:"auto"}}>
          ...
          {ex.duration} Min &nbsp;·&nbsp; 👥 {ex.minPlayers}–{ex.maxPlayers}
        </span>
      </div>
      ...
      {ex.setup&&<div style={{marginBottom:16}}>
      ...
      {ex.description&&<div style={{marginBottom:16}}>
      ...
      {ex.material?.length>0&&<div style={{marginBottom:16}}>
        <div style={{fontSize:12,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:.6,marginBottom:6}}>📦 Material</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
          {ex.material.map(m=><span key={m} style={{padding:"4px 10px",borderRadius:20,background:C.accentL,color:C.primary,fontSize:12,fontWeight:700}}>📦 {m}</span>)}
        </div>
      </div>}
      {ex.tags?.length>0&&<div style={{marginBottom:16}}>
      ...
      {ex.notes&&<div style={{marginBottom:16}}>
      ...
      <div style={{fontSize:11,color:"#cbd5e1"}}>Erstellt: {fmtDate(ex.createdAt)}</div>
...
// EXERCISE CARD
// ─────────────────────────────────────────────────────────────────
function ExCard({exercise:ex, onClick}) {
  return (
    <div onClick={onClick}
      style={{background:C.card,borderRadius:12,border:`1.5px solid ${C.border}`,padding:"14px 16px",cursor:"pointer",display:"flex",flexDirection:"column",gap:8,transition:"box-shadow .15s,transform .1s"}}
      ...
      {ex.tags?.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:4}}>
        ...
        {ex.tags.length>3&&<span style={{padding:"2px 8px",borderRadius:20,background:"#f1f5f9",color:C.muted,fontSize:11}}>+{ex.tags.length-3}</span>}
      </div>}
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{display:"flex",gap:10,marginBottom:12,flexWrap:"wrap"}}>
        ...
        <Btn onClick={()=>setShowFilter(f=>!f)} variant="secondary" sm>
          <Filter size={14}/> Filter {(filterCat||filterTag||filterRating)?`(aktiv)`:""}</Btn>
      </div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:16,overflowX:"auto",paddingBottom:4}}>
        {[["","Alle",exercises.length],...Object.entries(CATS).map(([k,v])=>[k,`${v.emoji} ${v.label}`,exercises.filter(e=>e.category===k).length])].map(([k,l,n])=>(
          <button key={k} onClick={()=>setFilterCat(k)}
            style={{padding:"5px 14px",borderRadius:20,border:`1.5px solid ${filterCat===k?(k?CATS[k].color:C.primary):C.border}`,
              background:filterCat===k?(k?CATS[k].bg:C.accentL):"white",
              color:filterCat===k?(k?CATS[k].color:C.primary):C.muted,
              cursor:"pointer",fontSize:13,fontWeight:700,whiteSpace:"nowrap",fontFamily:"inherit"}}>
            {l} ({n})
          </button>
        ))}
      </div>
      ...
      {filtered.length===0
        ?<EmptyState icon="📚" title={exercises.length===0?"Noch keine Übungen":"Nichts gefunden"}
            sub={exercises.length===0?"Lege deine erste Übung an.":"Andere Suchbegriffe versuchen."}
            onAdd={exercises.length===0?()=>setModal({type:"form",exercise:null}):undefined} addLabel="Erste Übung erstellen" />
        :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:12}}>
          {filtered.map(ex=><ExCard key={ex.id} exercise={ex} onClick={()=>setModal({type:"detail",exercise:ex})} />)}
        </div>
      }

      {modal?.type==="form"&&<Modal title={modal.exercise?"Übung bearbeiten":"Neue Übung"} onClose={()=>setModal(null)} wide>
        <ExerciseForm exercise={modal.exercise} onSave={(ex)=>{onSave(ex);setModal(null);}} onClose={()=>setModal(null)} />
      </Modal>}
      {modal?.type==="detail"&&<Modal title={modal.exercise.title} onClose={()=>setModal(null)} wide>
        <ExerciseDetail exercise={modal.exercise}
          onEdit={()=>setModal({type:"form",exercise:modal.exercise})}
          onDelete={onDelete} onClose={()=>setModal(null)} />
      </Modal>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
        <Inp label="Jahrgang" type="number" value={form.birthYear} onChange={e=>set("birthYear",Number(e.target.value))} min={2010} max={2030} style={{marginBottom:0}} />
      ...
      <div style={{marginBottom:14}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {[1,2,3,4].map(n=>{const s=STR[n]; return (
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
        <input type="checkbox" id="pactive" checked={form.active} onChange={e=>set("active",e.target.checked)} style={{width:16,height:16,cursor:"pointer"}} />
        <label htmlFor="pactive" style={{fontSize:14,fontWeight:600,color:C.text,cursor:"pointer"}}>Aktiv (nimmt am Training teil)</label>
      </div>
      <Txta label="Notizen" value={form.notes} onChange={e=>set("notes",e.target.value)} placeholder="Besonderheiten, Verletzungen..." rows={2} />
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",paddingTop:16,borderTop:`1px solid ${C.border}`}}>
        <Btn onClick={onClose} variant="secondary">Abbrechen</Btn>
        <Btn onClick={()=>{ if(!form.name.trim()) return; onSave({...form,id:form.id||uid(),createdAt:form.createdAt||now()}); }}>{player?.id?"Speichern":"Erstellen"}</Btn>
      </div>
    </div>
  );
}
...
function CoachForm({coach, onSave, onClose}) {
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",paddingTop:16,borderTop:`1px solid ${C.border}`}}>
        <Btn onClick={onClose} variant="secondary">Abbrechen</Btn>
        <Btn onClick={()=>{ if(!form.name.trim()) return; onSave({...form,id:form.id||uid(),createdAt:form.createdAt||now()}); }}>{coach?.id?"Speichern":"Erstellen"}</Btn>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      {tab==="players"&&(players.length===0
        ?<EmptyState icon="👦" title="Noch keine Spieler" onAdd={()=>setModal({type:"playerForm",data:null})} addLabel="Ersten Spieler anlegen" />
        :<div>
          {[4,3,2,1].map(s=>{
            const group=strGroups[s]; if(!group.length) return null;
            return <div key={s} style={{marginBottom:20}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                <StrBadge level={s} /><span style={{fontSize:13,color:C.muted}}>{group.length} Spieler</span>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
                {group.map(p=>(
                  <div key={p.id} style={{background:C.card,borderRadius:10,border:`1.5px solid ${C.border}`,padding:"12px 14px",opacity:p.active?1:.6}}>
                    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:6}}>
                      ...
                      <div style={{display:"flex",gap:4}}>
                        <button onClick={()=>setModal({type:"playerForm",data:p})} style={{background:"none",border:"none",cursor:"pointer",color:C.muted,padding:4}}><Edit2 size={14}/></button>
                        <button onClick={()=>setConfirmDel({type:"player",id:p.id,name:p.name})} style={{background:"none",border:"none",cursor:"pointer",color:"#ef4444",padding:4}}><Trash2 size={14}/></button>
                      </div>
                    </div>
      ...
      {confirmDel&&<Modal title="Löschen bestätigen" onClose={()=>setConfirmDel(null)}>
        <p style={{color:C.text,marginTop:0}}>„<strong>{confirmDel.name}</strong>" wirklich löschen?</p>
        <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
          <Btn onClick={()=>setConfirmDel(null)} variant="secondary">Abbrechen</Btn>
          <Btn onClick={()=>{ confirmDel.type==="player"?onDeletePlayer(confirmDel.id):onDeleteCoach(confirmDel.id); setConfirmDel(null); }} variant="danger"><Trash2 size={14}/> Löschen</Btn>
        </div>
      </Modal>}
      {modal?.type==="playerForm"&&<Modal title={modal.data?"Spieler bearbeiten":"Neuer Spieler"} onClose={()=>setModal(null)}>
        <PlayerForm player={modal.data} onSave={(p)=>{onSavePlayer(p);setModal(null);}} onClose={()=>setModal(null)} />
      </Modal>}
      {modal?.type==="coachForm"&&<Modal title={modal.data?"Trainer bearbeiten":"Neuer Trainer"} onClose={()=>setModal(null)}>
...
function TeamBuilderModal({availablePlayers, onSaveTeams, onClose}) {
  ...
  const modeBtn=(key,label,desc)=>(
    <button onClick={()=>setMode(key)} style={{flex:1,minWidth:120,padding:"8px 10px",borderRadius:8,border:`2px solid ${mode===key?C.primary:C.border}`,background:mode===key?C.accentL:"white",cursor:"pointer",textAlign:"left",fontFamily:"inherit"}}>
    ...
    <div>
      <div style={{marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <label style={{fontSize:12,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:.6}}>Spieler ({selected.length})</label>
          <div style={{display:"flex",gap:6}}>
            <Btn sm variant="secondary" onClick={()=>setSelectedIds(availablePlayers.filter(p=>p.active).map(p=>p.id))}>Alle</Btn>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:6,padding:10,background:"#f8fafc",borderRadius:8,border:`1.5px solid ${C.border}`}}>
          {availablePlayers.map(p=>(
            <button key={p.id} onClick={()=>togglePlayer(p.id)}
              style={{padding:"4px 12px",borderRadius:20,border:`2px solid ${selectedIds.includes(p.id)?STR[p.strength].color:C.border}`,background:selectedIds.includes(p.id)?STR[p.strength].light:"white",color:selectedIds.includes(p.id)?STR[p.strength].color:C.muted,cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:12,marginBottom:14}}>
        <div>
          ...
          <div style={{fontSize:11,color:C.muted,marginTop:4}}>→ {Math.floor(selected.length/perTeam)} Teams</div>
        </div>
        <div>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {modeBtn("balanced","⚖️ Ausgeglichen","Stärken gleichmäßig")}
            {modeBtn("mixed","🎨 Durchmischt","Alle Level je Team")}
      ...
      <Btn onClick={()=>setTeams(buildTeams(selected,perTeam,mode))} style={{width:"100%",justifyContent:"center",marginBottom:16}}>
              ...
              <div key={team.id} style={{background:C.card,borderRadius:10,border:`2px solid ${C.accent}`,padding:"12px 14px"}}>
                ...
                {team.players.map(p=>(
                  <div key={p.id} style={{display:"flex",alignItems:"center",gap:6,padding:"4px 0",borderBottom:`1px solid ${C.border}`}}>
                    <span>{STR[p.strength].emoji}</span>
                    <span style={{fontSize:13,fontWeight:600,color:C.text}}>{p.name}</span>
                  </div>
                ))}
                <div style={{fontSize:11,color:C.muted,marginTop:6}}>Ø {(team.players.reduce((s,p)=>s+p.strength,0)/(team.players.length||1)).toFixed(1)} Stärke</div>
              </div>
            ))}
          </div>
...
function SessionForm({session, players, coaches, exercises, onSave, onClose}) {
  const [form,setForm]=useState({date:todayISO(),duration:60,location:"",weather:"",coachIds:[],playerIds:[],exerciseIds:[],teams:[],notes:"",...session});
  const set=(k,v)=>setForm(f=>({...f,[k]:v}));
  const toggleArr=(k,id)=>set(k,form[k].includes(id)?form[k].filter(x=>x!==id):[...form[k],id]);

  const multiSelect=(items,selectedIds,toggle,label,render)=>(
    <div style={{marginBottom:14}}>
    ...
    <div>
      ...
      <div style={{marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          ...
          <div style={{display:"flex",gap:6}}>
            <Btn sm variant="secondary" onClick={()=>set("playerIds",players.filter(p=>p.active).map(p=>p.id))}>Alle</Btn>
            <Btn sm variant="secondary" onClick={()=>set("playerIds",[])}>Keine</Btn>
          </div>
        </div>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",paddingTop:16,borderTop:`1px solid ${C.border}`}}>
        <Btn onClick={onClose} variant="secondary">Abbrechen</Btn>
        <Btn onClick={()=>onSave({...form,id:form.id||uid(),createdAt:form.createdAt||now()})}>{session?.id?"Speichern":"Erstellen"}</Btn>
      </div>
    </div>
  );
}
...
function TrainingPage({sessions, players, coaches, exercises, onSaveSession, onDeleteSession}) {
  ...
  const getC=(id)=>coaches.find(c=>c.id===id);
  const getE=(id)=>exercises.find(e=>e.id===id);
  ...
  return (
    <div>
      ...
      {tab==="history"&&(sorted.length===0
        ?<EmptyState icon="📅" title="Noch kein Training" onAdd={()=>setModal({type:"session",data:null})} addLabel="Training planen" />
        :<div style={{display:"flex",flexDirection:"column",gap:12}}>
          {sorted.map(s=>{
            const present=(s.playerIds||[]).map(getP).filter(Boolean);
            const trainers=(s.coachIds||[]).map(getC).filter(Boolean);
            const exUsed=(s.exerciseIds||[]).map(getE).filter(Boolean);
            return (
              <div key={s.id} style={{background:C.card,borderRadius:12,border:`1.5px solid ${C.border}`,padding:"16px 20px"}}>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
                  ...
                  <div style={{display:"flex",gap:6}}>
                    <Btn sm variant="secondary" onClick={()=>setModal({type:"teamBuilder",data:{session:s,players:present.length?present:players.filter(p=>p.active)}})}>
                ...
                {present.length>0&&<div style={{marginTop:10}}>
                ...
                {exUsed.length>0&&<div style={{marginTop:10}}>
                ...
                {s.teams?.length>0&&<div style={{marginTop:10}}>
                  <div style={{fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",marginBottom:5}}>Teams</div>
                  <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                    {s.teams.map(t=><div key={t.id} style={{padding:"6px 12px",borderRadius:8,background:C.accentL,border:`1px solid ${C.accent}`}}>
                      <div style={{fontSize:12,fontWeight:700,color:C.primary}}>{t.name}</div>
      ...
      {tab==="teams"&&(
        <div style={{background:C.card,borderRadius:12,border:`1.5px solid ${C.border}`,padding:20}}>
          <h2 style={{margin:"0 0 6px",fontSize:18,fontWeight:800}}>Schnelle Teambildung</h2>
          <p style={{margin:"0 0 16px",color:C.muted,fontSize:14}}>Teams bilden ohne Training zu protokollieren.</p>
      ...
      {modal?.type==="session"&&<Modal title={modal.data?"Training bearbeiten":"Neues Training"} onClose={()=>setModal(null)} wide>
      ...
      {modal?.type==="teamBuilder"&&<Modal title="Teambildung" onClose={()=>setModal(null)} wide>
        <TeamBuilderModal availablePlayers={modal.data.players}
          onSaveTeams={(teams)=>{
...
function SettingsPage({exercises, players, coaches, sessions, onImport, toast}) {
  ...
  const ExCard2=({icon,title,desc,sub,onClick})=>(
  ...
  return (
    <div>
      ...
      <div style={{marginBottom:28}}>
        <h2 style={{fontSize:16,fontWeight:800,color:C.text,marginBottom:14,paddingBottom:8,borderBottom:`2px solid ${C.accentL}`}}>📤 Exportieren</h2>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <ExCard2 icon="💾" title="Vollständiges Backup" desc="Alle Daten: Übungen, Spieler, Trainer, Trainingsverlauf" sub={`${exercises.length} Übungen · ${players.length} Spieler · ${sessions.length} Trainings`}
            onClick={()=>{ dlJson({version:APP_VERSION,exportDate:new Date().toISOString(),type:"full",exercises,players,coaches,sessions},`gjugend_backup_${todayISO()}.json`); toast("Backup exportiert"); }} />
          <ExCard2 icon="📚" title="Nur Übungen" desc="Bibliothek teilen – ideal für andere Trainer" sub={`${exercises.length} Übungen`}
            onClick={()=>{ dlJson({version:APP_VERSION,exportDate:new Date().toISOString(),type:"exercises",exercises},`gjugend_uebungen_${todayISO()}.json`); toast("Übungen exportiert"); }} />
          <ExCard2 icon="👥" title="Team (JSON)" desc="Spieler & Trainer exportieren" sub={`${players.length} Spieler · ${coaches.length} Trainer`}
            onClick={()=>{ dlJson({version:APP_VERSION,exportDate:new Date().toISOString(),type:"team",players,coaches},`gjugend_team_${todayISO()}.json`); toast("Team exportiert"); }} />
          <ExCard2 icon="📊" title="Spieler (CSV)" desc="Spielerliste für Excel & Google Sheets" sub={`${players.length} Spieler`}
            onClick={()=>{ dlCsv(players,["name","birthYear","strength","active","jersey","notes"],`gjugend_spieler_${todayISO()}.csv`); toast("CSV exportiert"); }} />
        </div>
      </div>

      <div style={{marginBottom:28}}>
        <h2 style={{fontSize:16,fontWeight:800,color:C.text,marginBottom:14,paddingBottom:8,borderBottom:`2px solid ${C.accentL}`}}>📥 Importieren</h2>
        <div style={{background:C.card,borderRadius:10,border:`1.5px solid ${C.border}`,padding:"16px 18px"}}>
          ...
          <div style={{background:"#f8fafc",borderRadius:8,padding:"10px 14px",marginBottom:12,fontSize:12,color:C.muted}}>
            ...
            · JSON Backup (vollständig oder Teilexport)<br/>
      ...
      <div style={{marginBottom:28}}>
        <h2 style={{fontSize:16,fontWeight:800,color:C.text,marginBottom:14,paddingBottom:8,borderBottom:`2px solid ${C.accentL}`}}>🤖 KI-Integration</h2>
        <div style={{background:"#f8fafc",borderRadius:10,border:`1.5px dashed ${C.border}`,padding:"16px 18px",opacity:.6}}>
          ...
          <div style={{fontSize:13,color:C.muted,marginBottom:10}}>KI-gestützte Trainingsplanung und Übungsimport. Kommt in Build 2.</div>
      ...
      <div style={{textAlign:"center",padding:"20px 0",color:"#cbd5e1",fontSize:12}}>
        {APP_NAME} v{APP_VERSION} · Made with ⚽ for G-Jugend Trainer in Hamburg
      </div>
    </div>
  );
}
...
function Nav({page, setPage, counts}) {
  ...
  return (
    <>
      <style>{`
        .gjc-nav{position:fixed;left:0;top:0;bottom:0;width:200px;background:${C.nav};display:flex;flex-direction:column;z-index:100;padding:0 12px 20px}
        .gjc-main{margin-left:200px;padding:28px;max-width:1100px}
        .gjc-mobile-nav{display:none;position:fixed;bottom:0;left:0;right:0;background:${C.nav};z-index:100;border-top:1px solid rgba(255,255,255,.1)}
        @media(max-width:640px){
          .gjc-nav{display:none}
          .gjc-mobile-nav{display:flex}
          .gjc-main{margin-left:0!important;padding:16px;padding-bottom:80px}
        }
        @keyframes slideIn{from{opacity:0;transform:translateX(20px)}to{opacity:1;transform:translateX(0)}}
      `}</style>

      <div className="gjc-nav">
        ...
        {items.map(({key,icon:Icon,label,count})=>(
          <button key={key} onClick={()=>setPage(key)}
            style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,border:"none",cursor:"pointer",marginBottom:4,width:"100%",textAlign:"left",fontFamily:"inherit",
              background:page===key?"rgba(34,197,94,.2)":"transparent",color:page===key?"#4ade80":"rgba(255,255,255,.6)"}}>
...
export default function App() {
  ...
  const mergeArr=(existing,incoming)=>{
    const map=Object.fromEntries(existing.map(e=>[e.id,e]));
    incoming?.forEach(item=>{ if(!map[item.id]) map[item.id]=item; });
    return Object.values(map);
  };

  const saveExercise=(ex)=>{ const exists=exercises.find(e=>e.id===ex.id); if(exists){setExercises(exercises.map(e=>e.id===ex.id?ex:e));toast("Übung gespeichert");}else{setExercises([...exercises,ex]);toast("Übung erstellt");} };
  const deleteExercise=(id)=>{ setExercises(exercises.filter(e=>e.id!==id)); toast("Übung gelöscht"); };
  const savePlayer=(p)=>{ const exists=players.find(x=>x.id===p.id); if(exists){setPlayers(players.map(x=>x.id===p.id?p:x));toast("Spieler gespeichert");}else{setPlayers([...players,p]);toast("Spieler hinzugefügt");} };
  const deletePlayer=(id)=>{ setPlayers(players.filter(p=>p.id!==id)); toast("Spieler gelöscht"); };
  const saveCoach=(c)=>{ const exists=coaches.find(x=>x.id===c.id); if(exists){setCoaches(coaches.map(x=>x.id===c.id?c:x));toast("Trainer gespeichert");}else{setCoaches([...coaches,c]);toast("Trainer hinzugefügt");} };
  const deleteCoach=(id)=>{ setCoaches(coaches.filter(c=>c.id!==id)); toast("Trainer gelöscht"); };
  const saveSession=(s)=>{ const exists=sessions.find(x=>x.id===s.id); if(exists){setSessions(sessions.map(x=>x.id===s.id?s:x));toast("Training gespeichert");}else{setSessions([...sessions,s]);toast("Training protokolliert");} };
  ...
  const handleImport=(data, mode)=>{
    if(mode==="merge"){
      ...
      setPlayers(mergeArr(players,data.players));
    } else if(mode==="replace_players"){
      setPlayers(data.players||[]);
    }
  };

  if(!exReady||!plReady||!coReady||!seReady) return (
