...
function LibraryPage({exercises,onSave,onDelete,apiKey,toast}) {
  ...
  const printExercises=(exList)=>{
    ...
    const exHtml=exList.map(ex=>{
      ...
      const img=ex.imageUrl?`<img src="${ex.imageUrl}" style="width:100%;max-height:400px;object-fit:contain;display:block;background:#f8fafc;border-radius:6px;margin:8px 0;border:1px solid #e5e7eb" onerror="this.style.display='none'"/>`:"";
...
function printSession(s,exercises,toast) {
  ...
  const exHtml=ex.map(e=>{
    ...
    const img=e.imageUrl?`<img src="${e.imageUrl}" style="width:100%;max-height:400px;object-fit:contain;display:block;background:#f8fafc;border-radius:6px;margin:8px 0;border:1px solid #e5e7eb" onerror="this.style.display='none'"/>`:"";
