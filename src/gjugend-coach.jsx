...
import { BookOpen, Users, CalendarDays, Settings, Plus, Search, Edit2, Trash2, Download, Upload, Shuffle, Filter, Clock, Trophy, Bot, RefreshCw } from "lucide-react";
...
const dlJson = (o,n)=>{ const b=new Blob([JSON.stringify(o,null,2)],{type:"application/json"}),u=URL.createObjectURL(b),a=document.createElement("a");a.href=u;a.download=n;a.click();URL.revokeObjectURL(u); };
const dlCsv  = (r,c,n)=>{ const csv=[c.join(","),...r.map(x=>c.map(k=>`"${String(x[k]??"").replace(/"/g,'""')}"`).join(","))].join("\n"),b=new Blob(["\uFEFF"+csv],{type:"text/csv;charset=utf-8;"}),u=URL.createObjectURL(b),a=document.createElement("a");a.href=u;a.download=n;a.click();URL.revokeObjectURL(u); };
...
function useStorage(key,def) {
  ...
  useEffect(()=>{ (async()=>{ try{const r=await window.storage.get(key);if(r)setData(JSON.parse(r.value));}catch{}setReady(true); })(); },[key]);
...
// ── LIBRARY PAGE ──────────────────────────────────────────────────
function LibraryPage({exercises,onSave,onDelete,apiKey}) {
  ...
  return(<div>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:12}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        <Btn onClick={()=>setModal({type:"ai"})} variant="ai" sm><Bot size={14}/> KI-Import</Btn>
        <Btn onClick={()=>setModal({type:"form",ex:null})}><Plus size={16}/> Neue Übung</Btn>
      </div>
    </div>
    ...
    {filtered.length===0?<Empty icon="📚" title={exercises.length===0?"Noch keine Übungen":"Nichts gefunden"} sub={exercises.length===0?"Lege deine erste Übung an oder nutze KI-Import.":"Andere Suchbegriffe versuchen."} onAdd={exercises.length===0?()=>setModal({type:"form",ex:null}):undefined} addLabel="Erste Übung erstellen"/>
      :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:12}}>
        {filtered.map(ex=>(
          <div key={ex.id} onClick={()=>setModal({type:"detail",ex})} style={{background:C.card,borderRadius:12,border:`1.5px solid ${C.border}`,padding:"14px 16px",cursor:"pointer",display:"flex",flexDirection:"column",gap:8,transition:"box-shadow .15s"}} onMouseEnter={e=>e.currentTarget.style.boxShadow="0 4px 20px rgba(0,0,0,.1)"} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
    ...
    {modal?.type==="form"&&<Modal title={modal.ex?"Übung bearbeiten":"Neue Übung"} onClose={()=>setModal(null)} wide><ExerciseForm exercise={modal.ex} onSave={ex=>{onSave(ex);setModal(null);}} onClose={()=>setModal(null)}/></Modal>}
...
function PlayerForm({player,onSave,onClose}) {
  ...
  return(<div>
    ...
    <div style={{display:"flex",gap:10,justifyContent:"flex-end",paddingTop:16,borderTop:`1px solid ${C.border}`}}><Btn onClick={onClose} variant="secondary">Abbrechen</Btn><Btn onClick={()=>{if(!form.name.trim())return;onSave({...form,id:form.id||uid(),createdAt:form.createdAt||now()});}}>{player?.id?"Speichern":"Erstellen"}</Btn></div>
  </div>);
}
...
function CoachForm({coach,onSave,onClose}) {
  ...
  return(<div>
    ...
    <Txta label="Notizen" value={form.notes} onChange={e=>set("notes",e.target.value)} rows={2}/>
    <div style={{display:"flex",gap:10,justifyContent:"flex-end",paddingTop:16,borderTop:`1px solid ${C.border}`}}><Btn onClick={onClose} variant="secondary">Abbrechen</Btn><Btn onClick={()=>{if(!form.name.trim())return;onSave({...form,id:form.id||uid(),createdAt:form.createdAt||now()});}}>{coach?.id?"Speichern":"Erstellen"}</Btn></div>
  </div>);
}
...
function TeamPage({players,coaches,onSavePlayer,onDeletePlayer,onSaveCoach,onDeleteCoach}) {
  ...
  return(<div>
    ...
    <div style={{display:"flex",gap:4,background:"#f1f5f9",borderRadius:10,padding:4,marginBottom:20,width:"fit-content"}}>{tb("players","Spieler",players.length)}{tb("coaches","Trainer",coaches.length)}</div>
    {tab==="players"&&(players.length===0?<Empty icon="👦" title="Noch keine Spieler" onAdd={()=>setModal({type:"pf",data:null})} addLabel="Ersten Spieler anlegen"/>:
      <div>{[4,3,2,1].map(s=>{const g=grp[s];if(!g.length)return null;return(<div key={s} style={{marginBottom:20}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
          {g.map(p=><div key={p.id} style={{background:C.card,borderRadius:10,border:`1.5px solid ${C.border}`,padding:"12px 14px",opacity:p.active?1:.6}}>
            <div style={{display:"flex",justifyContent:"space-between",gap:6}}><div><div style={{fontWeight:800,fontSize:15,color:C.text}}>{p.name}</div>{p.jersey&&<div style={{fontSize:12,color:C.muted}}>#{p.jersey}</div>}{!p.active&&<div style={{fontSize:11,color:"#f59e0b",fontWeight:700}}>Inaktiv</div>}</div><div style={{display:"flex",gap:4}}><button onClick={()=>setModal({type:"pf",data:p})} style={{background:"none",border:"none",cursor:"pointer",color:C.muted,padding:4}}><Edit2 size={14}/></button><button onClick={()=>setDel({type:"player",id:p.id,name:p.name})} style={{background:"none",border:"none",cursor:"pointer",color:"#ef4444",padding:4}}><Trash2 size={14}/></button></div></div>
    ...
    {tab==="coaches"&&(coaches.length===0?<Empty icon="🧑‍🏫" title="Noch keine Trainer" onAdd={()=>setModal({type:"cf",data:null})} addLabel="Ersten Trainer anlegen"/>:
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:12}}>
        {coaches.map(c=><div key={c.id} style={{background:C.card,borderRadius:10,border:`1.5px solid ${C.border}`,padding:"14px 16px",opacity:c.active?1:.6}}>
          <div style={{display:"flex",justifyContent:"space-between"}}><div><div style={{fontWeight:800,fontSize:15,color:C.text}}>{c.name}</div><div style={{fontSize:12,color:C.muted}}>{ROLES[c.role]}</div>{c.phone&&<div style={{fontSize:12,color:C.muted}}>{c.phone}</div>}</div><div style={{display:"flex",gap:4}}><button onClick={()=>setModal({type:"cf",data:c})} style={{background:"none",border:"none",cursor:"pointer",color:C.muted,padding:4}}><Edit2 size={14}/></button><button onClick={()=>setDel({type:"coach",id:c.id,name:c.name})} style={{background:"none",border:"none",cursor:"pointer",color:"#ef4444",padding:4}}><Trash2 size={14}/></button></div></div>
        </div>)}
      </div>)}
    ...
    {modal?.type==="pf"&&<Modal title={modal.data?"Spieler bearbeiten":"Neuer Spieler"} onClose={()=>setModal(null)}><PlayerForm player={modal.data} onSave={p=>{onSavePlayer(p);setModal(null);}} onClose={()=>setModal(null)}/></Modal>}
    {modal?.type==="cf"&&<Modal title={modal.data?"Trainer bearbeiten":"Neuer Trainer"} onClose={()=>setModal(null)}><CoachForm coach={modal.data} onSave={c=>{onSaveCoach(c);setModal(null);}} onClose={()=>setModal(null)}/></Modal>}
  </div>);
}
...
function SessionForm({session,players,coaches,exercises,onSave,onClose}) {
  ...
  return(<div>
    ...
    <div style={{marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><label style={{fontSize:12,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:.6}}>Spieler ({form.playerIds.length})</label><div style={{display:"flex",gap:6}}><Btn sm variant="secondary" onClick={()=>set("playerIds",players.filter(p=>p.active).map(p=>p.id))}>Alle</Btn><Btn sm variant="secondary" onClick={()=>set("playerIds",[])}>Keine</Btn></div></div>
...
// ── TRAINING PAGE ─────────────────────────────────────────────────
function TrainingPage({sessions,players,coaches,exercises,onSaveSession,onDeleteSession,apiKey}) {
  ...
  return(<div>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:12}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        <Btn onClick={()=>setModal({type:"ai"})} variant="ai" sm><Bot size={14}/> KI-Trainingsplan</Btn>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {sorted.map(s=>{const pr=(s.playerIds||[]).map(gP).filter(Boolean),tr=(s.coachIds||[]).map(gC).filter(Boolean),ex=(s.exerciseIds||[]).map(gE).filter(Boolean);return(<div key={s.id} style={{background:C.card,borderRadius:12,border:`1.5px solid ${C.border}`,padding:"16px 20px"}}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
            <div><div style={{fontWeight:800,fontSize:16,color:C.text}}>{fmtDate(s.date)}</div><div style={{display:"flex",gap:12,color:C.muted,fontSize:13,marginTop:4,flexWrap:"wrap"}}><span>⏱ {s.duration} Min</span>{s.location&&<span>📍 {s.location}</span>}{s.weather&&<span>🌤 {s.weather}</span>}<span>👥 {pr.length}</span>{tr.length>0&&<span>🧑‍🏫 {tr.map(c=>c.name).join(", ")}</span>}</div></div>
          ...
          {ex.length>0&&<div style={{marginTop:10}}><div style={{fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",marginBottom:5}}>Übungen</div><div style={{display:"flex",flexWrap:"wrap",gap:4}}>{ex.map(e=><span key={e.id} style={{fontSize:12,padding:"2px 8px",borderRadius:20,background:"#f1f5f9",color:C.muted,fontWeight:600}}>{CATS[e.category]?.emoji} {e.title}</span>)}</div></div>}
...
export default function App() {
  ...
  const merge=(ex,inc)=>{ const m=Object.fromEntries(ex.map(e=>[e.id,e]));inc?.forEach(i=>{if(!m[i.id])m[i.id]=i;});return Object.values(m); };
  const saveEx=x=>{ exercises.find(e=>e.id===x.id)?setExercises(exercises.map(e=>e.id===x.id?x:e)):setExercises([...exercises,x]);toast(exercises.find(e=>e.id===x.id)?"Übung gespeichert":"Übung erstellt"); };
  const savePl=x=>{ players.find(e=>e.id===x.id)?setPlayers(players.map(e=>e.id===x.id?x:e)):setPlayers([...players,x]);toast("Spieler gespeichert"); };
  const saveCo=x=>{ coaches.find(e=>e.id===x.id)?setCoaches(coaches.map(e=>e.id===x.id?x:e)):setCoaches([...coaches,x]);toast("Trainer gespeichert"); };
  const saveSe=x=>{ sessions.find(e=>e.id===x.id)?setSessions(sessions.map(e=>e.id===x.id?x:e)):setSessions([...sessions,x]);toast("Training gespeichert"); };
  const saveTo=x=>{ tournaments.find(e=>e.id===x.id)?setTournaments(tournaments.map(e=>e.id===x.id?x:e)):setTournaments([...tournaments,x]);if(!tournaments.find(e=>e.id===x.id))toast("Turnier erstellt"); };
  const doImport=(data,mode)=>{
    if(mode==="merge"){if(data.exercises)setExercises(merge(exercises,data.exercises));if(data.players)setPlayers(merge(players,data.players));if(data.coaches)setCoaches(merge(coaches,data.coaches));if(data.sessions)setSessions(merge(sessions,data.sessions));if(data.tournaments)setTournaments(merge(tournaments,data.tournaments));}
    else if(mode==="replace"){if(data.exercises)setExercises(data.exercises);if(data.players)setPlayers(data.players);if(data.coaches)setCoaches(data.coaches);if(data.sessions)setSessions(data.sessions);if(data.tournaments)setTournaments(data.tournaments);}
    else if(mode==="merge_players")setPlayers(merge(players,data.players));
  ...
  return(<div style={{fontFamily:"system-ui,-apple-system,sans-serif",background:C.bg,minHeight:"100vh"}}>
    ...
    <main className="gm" style={{display:"block"}}>
      {page==="library"  &&<LibraryPage  exercises={exercises} onSave={saveEx} onDelete={id=>{setExercises(exercises.filter(e=>e.id!==id));toast("Übung gelöscht");}} apiKey={apiKey}/>}
      {page==="team"     &&<TeamPage     players={players} coaches={coaches} onSavePlayer={savePl} onDeletePlayer={id=>{setPlayers(players.filter(p=>p.id!==id));toast("Spieler gelöscht");}} onSaveCoach={saveCo} onDeleteCoach={id=>{setCoaches(coaches.filter(c=>c.id!==id));toast("Trainer gelöscht");}}/>}
      {page==="training" &&<TrainingPage sessions={sessions} players={players} coaches={coaches} exercises={exercises} onSaveSession={saveSe} onDeleteSession={id=>{setSessions(sessions.filter(s=>s.id!==id));toast("Training gelöscht");}} apiKey={apiKey}/>}
      {page==="turnier"  &&<TurnierPage  tournaments={tournaments} onSaveTournament={saveTo} onDeleteTournament={id=>{setTournaments(tournaments.filter(t=>t.id!==id));toast("Turnier gelöscht");}}/>}
